// ============================================================================
// NEXUS ARCADE UNIFIED ENGINE & CORE // ПОЛНЫЙ МОНОЛИТНЫЙ СКРИПТ (БЕЗ СОКРАЩЕНИЙ)
// ============================================================================
...
class SeniorSnake {
    ...
    spawnApple() {
        ...
        // Проверка, чтобы яблоко не спавнилось внутри змейки
    ...
    loop(timestamp) {
        ...
        if (timestamp - this.lastTick > this.speed) {
            ...
            if (head.x === this.apple.x && head.y === this.apple.y) {
                ...
                // Эффект сочных интерактивных частиц при съедании
                for (let i = 0; i < 15; i++) {
        ...
        this.ctx.fill();
        this.ctx.shadowBlur = 0; // Сброс размытия
...
class TapReactor {
    ...
    loop() {
        ...
        // Отрисовка активных энерго-кругов
        for (let i = this.circles.length - 1; i >= 0; i--) {
            let c = this.circles[i];
...
class BlockBlast {
    ...
    scanAndClearLines() {
        // Дополнительные бонусы за полную очистку горизонталей/вертикалей
        for (let r = 0; r < this.rows; r++) {
            let rowFull = true;
            for (let c = 0; c < this.cols; c++) {
                if (!this.grid[r][c]) rowFull = false;
            }
            if (rowFull) {
                this.score += 100;
    ...
    loop() {
        ...
        // Скорость появления блоков возрастает по мере игры
...
class CyberDash {
    ...
    switchGravity() {
        ...
        // iOS вспышка искр при инверсии гравитации
        for (let i = 0; i < 8; i++) {
    ...
    loop() {
        ...
        // Физика гравитационного смещения
        this.player.vy += this.player.gravity;
        this.player.y += this.player.vy;

        // Ограничения потолка и пола
        ...
        // Логика и отрисовка барьеров опасности
        this.ctx.fillStyle = '#FF375F';
        for (let i = this.obstacles.length - 1; i >= 0; i--) {
            let o = this.obstacles[i];
            o.x -= (4.5 + Math.min(3, this.score / 400)); // Плавный разгон скорости игры
            
            this.ctx.fillRect(o.x, o.y, o.w, o.h);

            // Высокоточная проверка коллизий (AABB Box)
        ...
        // Анимация хвоста частиц за игроком
...
function navigateTo(screenId) {
    ...
    if (targetScreen) targetScreen.add('active'); // Или targetScreen.classList.add('active') в зависимости от структуры верстки
    
    // Дублирующая безопасная проверка классов навигации
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Выход из аккаунта (Очистка кэша)
    ...
    // Навигационный док (Внизу экрана)
    ...
    // Экстренный выход из запущенной игры через кнопку "Назад"
...
function setupModeScreen() {
    ...
    // Автовыбор первого режима по умолчанию
...
function launchGame(game, mode) {
    // Безопасное уничтожение предыдущего движка во избежание утечек таймаутов
    if (STATE.currentGameEngine) {
        STATE.currentGameEngine.destroy();
        STATE.currentGameEngine = null;
    }
