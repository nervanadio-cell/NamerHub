// --- ХРАНИЛИЩЕ ДАННЫХ И СОСТОЯНИЯ (BULLETPROOF ARCHITECTURE) ---
let safeCoins = 100;
try {
...
try {
    ...
    // Безопасный перенос данных, если там лежала старая строка типа "#00EF72"
...
function setupModeScreen() {
    ...
    if (mTitle) mTitle.innerText = `Nexus // Выберите режим`;
...
class TapReactor {
    ...
    destroy() { this.alive = false; this.canvas.removeEventListener('pointerdown', this.tp); }
}
...
class BlockBlast {
    ...
    destroy() { this.alive = false; this.canvas.removeEventListener('pointerdown', this.cl); }
}
